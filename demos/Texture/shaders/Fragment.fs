...
for(int i=0; i<7; ++i)
{
    ...
    H = normalize(hf.xyz);
    // ambient just added
    ambient = texture2D(tex,texCord) *gl_LightSource[i].ambient;
    // calculate diffuse based on Lambert's law (L.N)
    diffuse = texture2D(tex,texCord)  *gl_LightSource[i].diffuse * max(dot(L, N), 0.0);
